...
export default class Priority extends Command {
  ...
  run = async (interaction: CommandInteraction) => {
    ...
    if (interaction.options.getSubcommand(true) === "add") {
      ...
      const hasPriority = await this.client.db.vrp_priority.findUnique({
    ...
    if (interaction.options.getSubcommand(true) === "remove") {
      ...
      await this.client.db.vrp_priority.delete({
        where: { user_id: playerId },
      });
      return await interaction.reply({ embeds: [embed] });
    }
    ...
    if (interaction.options.getSubcommand(true) === "get") {
      const player = await this.client.db.vrp_priority.findUnique({
        where: { user_id: playerId },
      });
      if (!player) {
        const embed = new MessageEmbed().setColor(`DARK_BLUE`)
